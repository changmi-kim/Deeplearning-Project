...
def checkLeftRight(image, results, draw=False, display=False):
    ...
    if draw:
        ...
        cv2.line(output_image, (width//2, 0),(width//2, height), (0, 0, 255), 1)    # 화면에 선긋는 인수(화면, 점1, 점2, 색깔, 굵기)
...
cv2.namedWindow('Do you wanna play a game?', cv2.WINDOW_NORMAL)
...
while camera_video.isOpened():
    ...
    if results.pose_landmarks:
        ...
        if game_started:
            ...
            if (horizontal_position == 'Left' and x_pos_index != 0) or (horizontal_position == 'Center' and x_pos_index == 2):
                ...
                pyautogui.moveTo(960,540)
                pyautogui.moveTo(1660,540)
                x_pos_index +=1
            
            if checkHandsJoined(frame, results)[1] == 'Hands Join':
                
                pyautogui.press(keys='w', presses=1)

                # pyautogui.keyDown('w', 2)
                # pyautogui.keyUp('w')
    ...
    cv2.imshow('Do you wanna play a game?', frame)
